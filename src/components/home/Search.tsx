...
const SearchComponent = ({ type }: { type?: "dashboard" }) => {
  ...
  return (
    <>
      <div
        ref={backdropRef}
        className="w-screen h-screen fixed top-0 left-0 right-0 backdrop-blur-sm bg-black/20 opacity-0 z-[40]"
        onClick={clearSearch}
      ></div>

      <div
        ref={searchContainerRef}
        className="w-full relative md:flex mt-6 mb-4 justify-center px-5 items-center z-[50]"
      >
        <div className="flex relative group w-full md:max-w-2xl transition-all duration-300">
          <div
            ref={searchInputRef}
            className="w-full flex items-center relative rounded-full bg-white dark:bg-neutral-800 shadow-lg overflow-hidden transition-all duration-300"
          >
            <div
